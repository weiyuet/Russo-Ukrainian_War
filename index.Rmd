...
ggplot(world_map, aes(long, lat, group = group)) +
  ...
  geom_text(inherit.aes = FALSE, data = point_data, aes(long, lat, label = scales::comma(refugees), vjust = stat_vjust), size = 2.5, family = "Gill Sans", colour = "#1A5A9A") +
  #overlay country names
  geom_text(inherit.aes = FALSE, data = point_data, aes(long, lat, label = toupper(region), vjust = region_vjust, colour = name_colour), size = 2.8, family = "Gill Sans") +
  #Ukraine label
  geom_text(inherit.aes = FALSE, data = text_data, aes(long, lat, label = text), size = 6, family = "Gill Sans", colour = "#1A5A9A") +
  #Ukrainian cities
  geom_point(inherit.aes = FALSE, data = ukr_cities, aes(long, lat), colour = "1A5A9A") +
  geom_text(inherit.aes = FALSE, data = ukr_cities, aes(long, lat, label = name), size = 4, vjust = 1.7, family = "Gill Sans", colour = "1A5A9A") +
  ...
  labs(title = "NUMBER OF UKRAINIAN REFUGEES IN NEIGHBOURING COUNTRIES",
       caption = "Data from UNHCR as of 21 June, 2022\nGraphic: @weiyuet") +
  theme_void() +
  theme(text = element_text(family = "Gill Sans"),
        plot.margin = margin(r = 15, l = 15),
        plot.title = element_text(size = 14, family = "Gill Sans", margin = margin(b = 10)),
        plot.caption = element_text(size = 10)
        )

ggsave("figures/ukranian-refugees.png", width = 8, height = 4.5)
```
